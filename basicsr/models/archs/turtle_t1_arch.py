...
# Optimized StateAlignBlock for turtle_t1_arch.py
class StateAlignBlock(nn.Module):
    ...
    def zero_out_non_top_k(self, attn_matrix, k):
        """Keep only top-k values in the attention matrix."""
        top_values, top_indices = torch.topk(attn_matrix, k=k, dim=-1)
    ...
    def forward(self, x, k_cached=None, v_cached=None):
        ...
        # Process through window convolutions
        ...
        q = q.view(b, 1, -1, H * W).transpose(-2, -1)
        k = k.view(b, 1, -1, H * W).transpose(-2, -1)
        v = v.view(b, 1, -1, h * w // (self.window_size * self.window_size))
        
        # Normalize
        q = F.normalize(q, dim=-1)
        k = F.normalize(k, dim=-1)
        
        # Add cached history states
        if k_cached is not None and v_cached is not None:
        ...
        # Create or retrieve local attention mask
        if self.local_mask is None or self.local_mask.shape[-1] != H * W:
            self.local_mask = self.create_local_attention_mask(H, W, 4, x.device)
        
        # Apply top-k sparsity and local masking
        attn1 = self.zero_out_non_top_k(attn, 5)
        attn2 = attn * self.local_mask
        ...
        out = out.view(b * curr_num_frames, c, h // self.window_size, w // self.window_size)
        out = self.project_out(out)
        out = out.view(b, curr_num_frames, c, h // self.window_size, w // self.window_size)
